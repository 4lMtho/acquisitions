...
export const getUserById = async (req, res, next) => {
    try {
        ...
        if (!idValidation.success) {
            return res.status(400).json({ error: 'Validation failed', details: formatValidationError(idValidation.error) });
        }
...
export const updateUser = async (req, res, next) => {
    try {
        ...
        if (!bodyValidation.success) {
            return res.status(400).json({ error: 'Validation failed', details: formatValidationError(bodyValidation.error) });
        }
        ...
        logger.info(`User updated: id=${id} by userId=${authUser.id} role=${authUser.role}`);
...
export const deleteUser = async (req, res, next) => {
    try {
        ...
        logger.info(`User deleted: id=${id} by userId=${authUser.id} role=${authUser.role}`);
